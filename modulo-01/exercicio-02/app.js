...
- Ao invés de inserir manualmente a posição do último caractere, faça com que a obtenção da última letra funcione para qualquer palavra que a 
"randomWord" receber;
...
- Exiba, no console, a string que "summerMessage" (abaixo) armazena, mas com todas 
...
- Exiba, no console, a string que "winterMessage" (abaixo) armazena, mas com 
...
let newName = name[0].toLocaleUpperCase() + name.slice(1,6)+ ' ' +name[7].toLocaleUpperCase() + name.slice(8,12)
console.log(newName);
...
newName = `${name[0].toLocaleUpperCase()}${name.slice(1,6)} ${name[7].toLocaleUpperCase()}${name.slice(8)}`
